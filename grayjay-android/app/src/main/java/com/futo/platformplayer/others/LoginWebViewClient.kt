...
class LoginWebViewClient : WebViewClient {
    ...
    override fun shouldInterceptRequest(view: WebView?, request: WebResourceRequest?): WebResourceResponse? {
        ...
        val completionUri = completionUrlToCheck?.let(Uri::parse)
        val youtubeGuideCompatibilityMatch = completionUri?.let { expected ->
            expected.path == "/youtubei/v1/guide" &&
                expected.host.orEmpty().endsWith(".youtube.com", ignoreCase = true) &&
                request.url.host.orEmpty().endsWith(".youtube.com", ignoreCase = true) &&
                request.url.path == expected.path
        } ?: false
        ...
        ) || youtubeGuideCompatibilityMatch;
