...
class JSHttpClient : ManagedHttpClient {
    ...
    fun applyHeaders(url: Uri, headers: MutableMap<String, String>, applyAuth: Boolean = false, applyOtherCookies: Boolean = false) {
        ...
        if (applyAuth && auth != null) {
            ...
            createYoutubeAuthorization(domain, findHeader(headers, "x-origin"))?.let {
    ...
    override fun beforeRequest(request: okhttp3.Request): okhttp3.Request {
        ...
        if (auth != null) {
            ...
            createYoutubeAuthorization(domain, request.header("x-origin"))?.let {
    ...
    /**
     * YouTube's WebView login exposes its session cookies reliably, but no longer consistently
     * exposes its generated Authorization header to shouldInterceptRequest. Generate the same
     * short-lived SAPISID signatures at request time instead of persisting a stale browser header.
     */
    private fun createYoutubeAuthorization(domain: String, requestedOrigin: String?): String? {
        if (domain != "youtube.com" && !domain.endsWith(".youtube.com")) return null

        val cookies = hashMapOf<String, String>()
        synchronized(_currentCookieMap) {
            _currentCookieMap
                .filter { domain.matchesDomain(it.key) }
                .forEach { (_, values) -> cookies.putAll(values) }
        }
        val secureOne = cookies["__Secure-1PAPISID"]
        val secureThree = cookies["__Secure-3PAPISID"]
        val primary = cookies["SAPISID"] ?: secureThree ?: secureOne ?: return null
        ...
        return buildList {
            add(createSapisidToken("SAPISIDHASH", primary, origin, timestamp))
            secureOne?.let { add(createSapisidToken("SAPISID1PHASH", it, origin, timestamp)) }
            secureThree?.let { add(createSapisidToken("SAPISID3PHASH", it, origin, timestamp)) }
        }.joinToString(" ")
    }

    private fun createSapisidToken(prefix: String, cookie: String, origin: String, timestamp: Long): String {
        val digest = MessageDigest.getInstance("SHA-1")
            .digest("$timestamp $cookie $origin".toByteArray(Charsets.UTF_8))
            .joinToString("") { "%02x".format(it) }
        return "$prefix ${timestamp}_$digest"
    }
